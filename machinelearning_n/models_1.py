...
class RegressionModel(Model):
    ...
    def run(self, x, y=None):
        ...
        self.negator = nn.Input(self.graph1, np.array([[-1]]))
        self.negated_x = nn.Input(self.graph1, np.array([[-1]])*x)
        if y is not None:
            ...
            ng_xw_1 = nn.MatrixMultiply(self.graph1, ng_x, self.w1)
            ...
            ng_relu_l2 = nn.ReLU(self.graph1, ng_l1w2_ng_plus_b2)
            ng_l2w3 = nn.MatrixMultiply(self.graph1, relu_l2, self.w3)
            ...
            odd_f = nn.MatrixVectorAdd(self.graph1, l2w3_plus_b3, inverse_ng_l2w3_plus_b3)
...
class OddRegressionModel(Model):
    ...
    def run(self, x, y=None):
        ...
        if y is not None:
            ...
            ng_xw_1 = nn.MatrixMultiply(graph2, ng_x, self.w1)
            ...
            ng_relu_l2 = nn.ReLU(graph2, ng_l1w2_ng_plus_b2)
            ng_l2w3 = nn.MatrixMultiply(graph2, relu_l2, self.w3)
            ng_l2w3_plus_b3 = nn.MatrixVectorAdd(graph2, ng_l2w3, self.b3)

            inverse_ng_l2w3_plus_b3 = nn.MatrixMultiply(graph2, ng_l2w3_plus_b3, self.negator)
            odd_f = nn.MatrixVectorAdd(self.graph1, l2w3_plus_b3, inverse_ng_l2w3_plus_b3)
            return 
            "*** YOUR CODE HERE ***"
