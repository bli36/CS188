...
class RegressionModel(Model):
    ...
    def __init__(self):
        ...
        self.w2 = nn.Variable(200,300)
        self.w3 = nn.Variable(300,1)

        self.b1 = nn.Variable(1, 200)
        self.b2 = nn.Variable(1,300)
    ...
    def run(self, x, y=None):
        ...
        if y is not None:
            ...
            loss = nn.SquareLoss(self.graph1, l2w3_plus_b3, self.input_y)
        ...
        # Then we use the graph to perform backprop and update our variables
            ...
            "*** YOUR CODE HERE ***"
            # print(self.graph1.get_output(self.last_node))
...
class OddRegressionModel(Model):
    ...
    def __init__(self):
        ...
        self.w2 = nn.Variable(100,300)
        self.w3 = nn.Variable(300,1)

        self.b1 = nn.Variable(1, 100)
        self.b2 = nn.Variable(1,300)
        self.b3 = nn.Variable(1,1)
    ...
    def run(self, x, y=None):
        ...
        self.graph1 = nn.Graph([self.w1,self.b1,self.w2,self.b2,self.w3,self.b3])
        self.input_x = nn.Input(self.graph1, x)
        self.negated_x = nn.Input(self.graph1, np.array([[float(-1)]])*x)
        self.negator = nn.Input(self.graph1, np.array([[float(-1)]]))
        ...
        if y is not None:
            ...
            l2w3_plus_b3 = nn.MatrixVectorAdd(self.graph1, l2w3, self.b3)
            # loss = nn.SquareLoss(self.graph1, l2w3_plus_b3, self.input_y)
            # ng_x = nn.MatrixMultiply(self.graph1, self.input_x, self.negator)
            ng_xw_1 = nn.MatrixMultiply(self.graph1, self.negated_x, self.w1)
            ng_xw1_plus_b1 = nn.MatrixVectorAdd(self.graph1, ng_xw_1, self.b1)
            ng_relu_l1 = nn.ReLU(self.graph1, ng_xw1_plus_b1)
            ng_l1w2 = nn.MatrixMultiply(self.graph1, ng_relu_l1, self.w2)
            ng_l1w2_plus_b2 = nn.MatrixVectorAdd(self.graph1, ng_l1w2, self.b2)
            ng_relu_l2 = nn.ReLU(self.graph1, ng_l1w2_plus_b2)
            ng_l2w3 = nn.MatrixMultiply(self.graph1, ng_relu_l2, self.w3)
            ng_l2w3_plus_b3 = nn.MatrixVectorAdd(self.graph1, ng_l2w3, self.b3)

            inverse_ng_l2w3_plus_b3 = nn.MatrixMultiply(self.graph1, ng_l2w3_plus_b3, self.negator)

            odd_f = nn.Add(self.graph1, l2w3_plus_b3, inverse_ng_l2w3_plus_b3)
            loss = nn.SquareLoss(self.graph1, odd_f, self.input_y)
            return self.graph1
        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            "*** YOUR CODE HERE ***"
            graph2 = nn.Graph([self.w1,self.b1,self.w2,self.b2,self.w3,self.b3])
            negated_x = nn.Input(graph2, np.array([[float(-1)]])*x)
            negator = nn.Input(graph2, np.array([[float(-1)]]))
            input_x = nn.Input(graph2, x)
            xw_1 = nn.MatrixMultiply(graph2, input_x, self.w1)
            xw1_plus_b1 = nn.MatrixVectorAdd(graph2, xw_1, self.b1)
            relu_l1 = nn.ReLU(graph2, xw1_plus_b1)
            ...
            # ng_x = nn.MatrixMultiply(graph2, self.input_x, self.negator)
            ng_xw_1 = nn.MatrixMultiply(graph2, negated_x, self.w1)
            ng_xw1_plus_b1 = nn.MatrixVectorAdd(graph2, ng_xw_1, self.b1)
            ng_relu_l1 = nn.ReLU(graph2, ng_xw1_plus_b1)
            ng_l1w2 = nn.MatrixMultiply(graph2, ng_relu_l1, self.w2)
            ng_l1w2_plus_b2 = nn.MatrixVectorAdd(graph2, ng_l1w2, self.b2)
            ng_relu_l2 = nn.ReLU(graph2, ng_l1w2_plus_b2)
            ng_l2w3 = nn.MatrixMultiply(graph2, ng_relu_l2, self.w3)
            ng_l2w3_plus_b3 = nn.MatrixVectorAdd(graph2, ng_l2w3, self.b3)
 
            inverse_ng_l2w3_plus_b3 = nn.MatrixMultiply(graph2, ng_l2w3_plus_b3, negator)
            odd_f = nn.Add(graph2, l2w3_plus_b3, inverse_ng_l2w3_plus_b3)
            return graph2.get_output(odd_f)
...
class DigitClassificationModel(Model):
    ...
    def __init__(self):
        ...
        self.w2 = nn.Variable(500,400)
        self.w3 = nn.Variable(400,10)

        self.b1 = nn.Variable(1, 500)
        self.b2 = nn.Variable(1,400)
...
class LanguageIDModel(Model):
    ...
    def run(self, xs, y=None):
        ...
        print('btach',batch_size)

        "*** YOUR CODE HERE ***"
        
        
        self.graph1 = nn.graph([self.Hi, self.w])


        if y is not None:
            "*** YOUR CODE HERE ***"
            for i in range(len(xs)):
                self.ci = nn.Input(self.graph1, xs[i]) #xs[i] = (batch_size, d=num_char)
                ci_wi = nn.MatrixMultiply(self.graph1, self.ci, self.w)
                cwi_hi = nn.MatrixMultiply(self.graph1, self.ci_hi, self.Hi)
